...
module.exports = (app) => {
  ...
  app.get("/banks/balance", async (req, res) => {
    try {
      ...
      let startDate = new Date(isNaN(Date.parse(req.query.start)) ? '1970/01/01' :  req.query.start).toISOString();
      let endDate = new Date(isNaN(Date.parse(req.query.end)) ? Date.now() : req.query.end).toISOString();
