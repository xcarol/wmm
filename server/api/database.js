...
const queryUpdateTransactionsByFilter =
  "UPDATE transactions t SET t.category = ':category' \
    FROM transactions t \
    INNER JOIN filters f ON t.description LIKE CONCAT('%', :filter, '%') \
    WHERE t.category = '' AND f.category = ':category'; ";
...
async function updateTransactionsByFilter(filter) {
  try {
    const connection = await getConnection();
    const result = await connection.query(
      queryUpdateTransactionsByFilter.replace("%1", filter)
    );
    connection.close();
    return result;
  } catch (err) {
